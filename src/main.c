...
void read_in_domain_pattern(char* ffile){
  FILE *file = fopen(ffile, "r");
    ...
    rwopt_len = 0;
    short int domain, op, bank;
    while (fscanf(file, "%hd %hd %hd", &domain, &op, &bank) != EOF) {
        rwopt_len++;
    }
    fclose(file);

    file = fopen(ffile, "r");
    rwopt_buffer = (rwopt_t *)malloc(sizeof(rwopt_t) * rwopt_len);
    //file format -> domain, op, bank
    int i = 0;
    while (fscanf(file, "%hd %hd %hd", &domain, &op, &bank) != EOF) {
        rwopt_t* rwopt = (rwopt_t *)malloc(sizeof(rwopt_t));
        rwopt->domain_id = domain;
        rwopt->op = op;
        rwopt_buffer[i] = *rwopt;
        i++;
    }
    fclose(file);
    return;
...
int main(int argc, char *argv[]) {
  ...
  if(RWOPT == 1){
    read_in_domain_pattern(argv[argc-1]);
  }
  if (!vi_file){
    vi_file = fopen("../input/4Gb_x4.vi", "r");
    /* Find the appropriate .vi file to read*/
  ...
  unsigned int i = log_base2(NUM_CHANNELS) + log_base2(NUM_RANKS) +
          log_base2(NUM_BANKS) + log_base2(NUM_ROWS) + log_base2(NUM_COLUMNS) +
          log_base2(CACHE_LINE_SIZE);
        ...
        while ((num_fetch < MAX_FETCH) && (ROB[numc].inflight != ROBSIZE) &&
          ...
          if (nonmemops[numc]) { /* Have some non-memory-ops to consume. */
            ...
            if (opertype[numc] == 'R') {
              ...
              if(domain_id[numc] >= DOMAIN_COUNT){
                printf("Panic. Poor trace format. Too many domains for spec\n");
                return -7;
              }
              ...
              if (SECURED == 0){
                ...
                if (lat) {
                 ...
                 if(secure_read_queue_length[0][domain_id[numc]] + secure_write_queue_length[0][domain_id[numc]] <= WQ_CAPACITY){
                    insert_read_secure(addr[numc], CYCLE_VAL, numc, ROB[numc].tail,
                                      instrpc[numc], domain_id[numc]);
                  }else {
                    //printf("Read Queue Full\n");
                    //writeqfull = 1;
                    break;
                  }
                }
      ...
      if(num_done == NUMCORES){
        for (numch = 0; numch < NUM_CHANNELS; numch++) {
          for (int d = 0; d < DOMAIN_COUNT; d++){
            if (secure_write_queue_length[numch][d] != 0 && secure_read_queue_length[numch][d] != 0)
              break;
          }
        }
        if (numch == NUM_CHANNELS)
  ...
  for(int i = 0; i < NUM_BANKS; i++){
    printf("Bank %d: Writes: %d, Reads: %d\n", i, bank_writes[i], bank_reads[i]);
  }
  return 0;
}
