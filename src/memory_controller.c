...
// Have there been 3 or less activates in the last T_FAW period??? should be 4
...
void init_memory_controller_vars() {
  ...
  for (int i = 0; i < NUM_CHANNELS; i++) {
    ...
    for (int d = 0; d < DOMAIN_COUNT; d++) {
      ...
      domain_read_queues[i][d] = NULL;
      domain_write_queues[i][d] = NULL;
    }
...
dram_address_t *calc_dram_addr(long long int physical_address) {
  ...
  } else if (ADDRESS_MAPPING == 3) {
    //random mapping
    temp_b = input_a;
    input_a = input_a >> channelBitWidth;
    temp_a = input_a << channelBitWidth;
    this_a->channel = temp_a ^ temp_b;	// strip out the channel address
    this_a->rank = rand() % NUM_RANKS;	// strip out the rank address
    this_a->bank = rand() % NUM_BANKS;
    this_a->row = rand() % NUM_ROWS;
    this_a->column = rand() % NUM_COLUMNS;
  } else {
  ...
  //unique id for the bank within a rank
  this_a->bank_id = (this_a->rank * NUM_BANKS) + this_a->bank;
  return (this_a);
}
...
void *init_new_node(long long int physical_address, long long int arrival_time,
  ...
  if (new_node == NULL) {
    ...
    new_node->dram_addr.bank_id = this_node_addr->bank_id;
...
int read_matches_write_or_read_queue_secure(long long int physical_address, int domain_id) {
  ...
  LL_FOREACH(domain_read_queues[channel][domain_id], rd_ptr) {
    if (rd_ptr->dram_addr.actual_address == physical_address) {
      num_read_merge++;
      stats_reads_merged_per_channel[channel]++;
      return RQ_LOOKUP_LATENCY;
  ...
  LL_FOREACH(domain_write_queues[channel][domain_id], wr_ptr) {
    if (wr_ptr->dram_addr.actual_address == physical_address) {
      num_read_merge++;
      stats_reads_merged_per_channel[channel]++;
      return WQ_LOOKUP_LATENCY;
...
request_t *insert_write_secure(long long int physical_address,
  ...
  LL_APPEND(domain_write_queues[new_node->dram_addr.channel][domain_id], new_node);
...
request_t *insert_read_secure(long long int physical_address,
  ...
  //LL_APPEND(domain_queues[new_node->dram_addr.channel][domain_id], new_node);
  LL_APPEND(domain_read_queues[new_node->dram_addr.channel][domain_id], new_node);
...
void update_entire_queue_commands_secured(int channel){
  for(int d =0; d < DOMAIN_COUNT; d++){
    request_t *curr = NULL;
    LL_FOREACH(domain_write_queues[channel][d], curr) {
      if (curr->request_served == 1)
        continue;
    ...
    LL_FOREACH(domain_read_queues[channel][d], curr) {
      // ignore the requests whose completion time has been determined
      // these requests will be removed this very cycle
      if (curr->request_served == 1)
        continue;
...
void clean_queues(int channel) {
  ...
  for(int doms = 0; doms < DOMAIN_COUNT; doms++){
    LL_FOREACH_SAFE(domain_read_queues[channel][doms], rd_ptr, rd_tmp){
      if(rd_ptr->request_served == 1){
        assert(rd_ptr->next_command == COL_READ_CMD);

        assert(rd_ptr->completion_time != -100);

        LL_DELETE(domain_read_queues[channel][doms], rd_ptr);
        ...
        secure_read_queue_length[channel][doms]--;

        assert(secure_read_queue_length[channel][doms] >= 0);
      }
    }
    LL_FOREACH_SAFE(domain_write_queues[channel][doms], wrt_ptr, wrt_tmp){
      if(wrt_ptr->request_served == 1){
        assert(wrt_ptr->next_command == COL_WRITE_CMD);

        LL_DELETE(domain_write_queues[channel][doms], wrt_ptr);
...
int issue_request_command(request_t *request) {
  ...
  case ACT_CMD:
    ...
    request->activation_cycle = CYCLE_VAL;
...
void issue_forced_refresh_commands(int channel, int rank) {
  ...
  //find any open row and addi to the close q
}
...
void print_stats(int channel) {
  ...
  printf("-------- Domain Stats-----------\n");
  for(int d = 0; d < DOMAIN_COUNT; d++){
    printf("Domain %d Fake Requests: %lld\n", d, fake_request_sent[d]);
  }
}
