...
typedef struct req {
  ...
  long long int activation_cycle;
} request_t;
...
typedef struct close_this{
  int rank;
  int bank;
  struct close_this *next;
} close_this_t;

typedef struct fs_data {
  long long last_req_issue_cycle;
  int current_turn_time;
  int domain_turn;
  int turn_counter;
  int bank_turn;
  int bank_used;
  int type_sent; // -1 nothing sent, 0 read, 1 write
  int rank_used;
  int domain_zero_starter;
  int deadtime;
  int next_op;
  optype_t buffer[MAX_DOMAIN_COUNT][100]; 
  int pointer;
  opt_queue_t *domain_reads;
  opt_queue_t *domain_writes;
  opt_queue_t *command_order;
  opt_queue_t *q_turns;
  int drain_writes;
  int rwopt_pointer;
  int window;
  int first_read_sent;
  int second_read_sent;
  int request_sent;
  int turn_len;
  int bubble;
  int prev;
...
typedef struct rwopt{
  short int domain_id;
  short int op;
} rwopt_t;

int rwopt_len;
rwopt_t *rwopt_buffer;

fs_data_t channel_fs_data[MAX_NUM_CHANNELS];
int second_used;

int total_request_sent[3];
int both_types_sent;
int reads_sent;
int writes_sent;
...
int bank_reads[MAX_NUM_BANKS];
int bank_writes[MAX_NUM_BANKS];
...
request_t *domain_queues[MAX_NUM_CHANNELS][MAX_DOMAIN_COUNT];
request_t *domain_read_queues[MAX_NUM_CHANNELS][MAX_DOMAIN_COUNT];
request_t *domain_write_queues[MAX_NUM_CHANNELS][MAX_DOMAIN_COUNT];
...
long long int secure_read_queue_length[MAX_NUM_CHANNELS][MAX_DOMAIN_COUNT];
long long int secure_write_queue_length[MAX_NUM_CHANNELS][MAX_DOMAIN_COUNT];
...
// functions
long long int fake_request_sent[MAX_DOMAIN_COUNT];
