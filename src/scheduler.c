  ...
  if (drain_writes[channel]) {
    ...
    LL_FOREACH(write_queue_head[channel], wr_ptr) {
      if (wr_ptr->command_issuable) {
        bank_writes[wr_ptr->dram_addr.bank]++;
        issue_request_command(wr_ptr);
        //printf("Domain: %d, Rank: %d, Bank:%d W\n", wr_ptr->domain_id, wr_ptr->dram_addr.rank, wr_ptr->dram_addr.bank);
        break;
      }
    }
    return;
  ...
  if (!drain_writes[channel]) {
    LL_FOREACH(read_queue_head[channel], rd_ptr) {
      if (rd_ptr->command_issuable) {
        bank_reads[rd_ptr->dram_addr.bank]++;
        issue_request_command(rd_ptr);
        //printf("Domain: %d, Rank: %d, Bank:%d R\n", rd_ptr->domain_id, rd_ptr->dram_addr.rank, rd_ptr->dram_addr.bank);
        break;
      }
    }
    return;
